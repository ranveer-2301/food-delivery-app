...
import { bannerAssets } from '../../assets/drive-download-20250620T152333Z-1-001/dummydata';
...
const Banner = () => {
  ...
  return (
    <div className='relative'>
      <div className='bg-gradient-to-br from-amber-800 to-amber-700 text-white py-16 px-4 sm:px-8 relative overflow-hidden'>
        ...
        <div className='max-w-6xl mx-auto flex flex-col md:flex-row items-center gap-12 relative z-10'>
          ...
          <div className='flex-1 space-y-8 relative md:pr-8 lg:pr-20 text-center md:text-left'>
            ...
            <p className='text-lg md:text-lg lg:text-xl font-playfair italic sm:text-xl text-amber-100 max-w-xl opacity-90 mx-auto md:mx-0'>
              Best cooks and best delivery guys all at your service. Hot tasty food will reach you in minutes...
            </p>
            ...
            <form onSubmit={handleSearch} className='relative max-w-2xl mx-auto md:mx-0 group'>
              <div className='relative flex items-center bg-amber-900/30 rounded-xl border-2 border-amber-500 shadow-2xl hover:bg-amber-400/50 transition-all duration-300'>
                ...
                <button
                  type="submit"
                  className="mx-4 px-6 py-3 bg-gradient-to-r from-amber-400 to-amber-300 
                    rounded-lg font-semibold text-amber-900
            ...
            <div className="flex flex-wrap gap-4 justify-center md:justify-start mt-6">
              <button className="group flex items-center gap-3 bg-amber-800/30 hover:bg-amber-800/50 px-6 py-3 rounded-xl transition-all duration-300 border-2 border-amber-700/50 hover:border-amber-500 backdrop-blur-sm">
                <FaDownload className='text-lg text-amber-400 group-hover:animate-bounce' />
                <span className='text-lg'>Download App</span>
              </button>
              <button
                onClick={() => setShowVideo(true)}
                className='group flex items-center gap-3 bg-gradient-to-r from-amber-400 to-amber-300 hover:from-amber-300 hover:to-amber-200 px-6 py-3 rounded-xl transition-all duration-300 shadow hover:shadow-amber-300/30'
              >
            ...
            {orbitImages.map((imgSrc, index) => (
              ...
              >
                <img
                  src={imgSrc}
                  alt={`Orbital ${index + 1}`}
                  className='w-full h-full rounded-full border-amber-500/30 shadow-lg bg-amber-900/20 p-1 object-cover'
                />
              </div>
            ))}
          </div>
