...
const Navbar = () => {
  ...
  // EXTRACT DESTOP AUTH BUTTON
  const renderDesktopAuthButton = () => {
    return isAuthenticated? (
      <button onClick={handleLogout} className='px-3 md:px-3 1g:px-6 py-1.5 md:py-2 1g:py-3 bg-gradient-to-br from-amber-500
      ...
      shadow-nd shadow-amber-900/20 text-xs md:text-sm 1g:text-sm'>
      <FiLogout className= 'text-base md:text-lg lg:text-lg' />
      <span className=' text-shadow'>Logout</span>
      </button>
      ):(
        <button onClick={() => navigate('/login')} className='px-3 md:px-3 1g:px-6 py-1.5 md:py-2 1g:py-3 bg-gradient-to-br from-amber-500
      to-amber-700 text-[#2D1B0E] rounded-2xl font-bold hover:shadow-lg hover:shadow-amber-600/40 transition-all
      transform hover:scale-[1.02] border-2 border-amber-600/20 flex items-center space-x-2
      shadow-nd shadow-amber-900/20 text-xs md:text-sm 1g:text-sm'>
  ...
  return (
    <nav className='bg-[#2D1B0E] border-b-8 border-amber-900/30 shadow-amber-900/30 sticky top-0 z-50 shadow-[0_25px_50px_-12px] font-vibes group/nav'>
      
      {/* Decorative line above navbar */}
        ...
        <div className='flex justify-between items-center h-16 md:h-20 lg:h-24'>
          ...
          <div className='flex items-center space-x-2'>
            ...
            <div className='ml-2'>
              <NavLink
                to='/'
                className='text-xl md:text-2xl lg:text-4xl bg-gradient-to-r from-amber-400 to-amber-600 bg-clip-text text-transparent font-monsieur tracking-wider whitespace-nowrap'
              >
          ...
          <div className='hidden md:flex items-center space-x-4'>
            {navLinks.map((link) => (
              ...
              >
                ...
                text-amber-500 group-hover:text-amber-300 transition-all'>{link.icon}</span>
            ...
            <div className='flex items-center space-x-2 md:space-x-3 lg:space-x-4ml-3
            md:ml-3 lg:ml-6 mr-2 md:mr-3 lg:mr-4'>
              ...
              hover:shadow-lg hover:shadow-amber-500/30 shadow-md shadow-amber-900/20'>
                ...
                {totalItems > 0 && (
                  ...
                  text-amber-100 text-xs w-5 rounded-full flex items-center justify-center'>
                    {totalItems}
                  </span>
                )}
              </NavLink>
        ...
        shadow-amber-900/30 w-full'>
          <div className='px-4 py-4 space-y-2'>
            ...
            <div className='pt-4 border-t-2 border-amber-900/30 space-y-2'>
              ...
              className='w-full px-4 py-3 text-center text-amber-100 rounded-xl border-2 border-amber-900/30 hover:border-amber-500/50 flex
      ...
      {/* LOGIN MODEL */}
      {showLoginModel && (
        <div className='fixed indert-0 bg-balck/60 flex items-center justify-center z-50 p-4'>
